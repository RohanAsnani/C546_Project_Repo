...
const exportedMethods = {
  async create(creationInfo) {
    ...
    creationInfo = {
      employeeId: creationInfo.employeeId,
      firstName: creationInfo.firstName,
      lastName: creationInfo.lastName,
      username: creationInfo.username,
      password: creationInfo.password,
      gender: creationInfo.gender,
      maritalStatus: creationInfo.maritalStatus,
      department: creationInfo.department,
      role: creationInfo.role,
      notes: creationInfo.notes,
      status: creationInfo.status,
      vet: creationInfo.vet,
      disablity: creationInfo.disability,
      race: creationInfo.race,
      countryOfOrigin: creationInfo.countryOfOrigin,
      startDate: creationInfo.startDate,
      endDate: creationInfo.endDate,
      dob: creationInfo.dob,
      currentPosition: creationInfo.currentPosition,
      currentSalary: creationInfo.currentSalary,
      promoDate: creationInfo.promoDate,
      previousPos: creationInfo.previousPos,
      contactInfo: {
        phone: creationInfo.phone,
        email: creationInfo.email,
        primaryAddress: creationInfo.primaryAddress,
        secondaryAddress: creationInfo.secondaryAddress,
      },
      subordinates: creationInfo.subordinates,
      managerId: creationInfo.managerId,
      leave: creationInfo.leave,
      leaveBank: creationInfo.leaveBank,
    };

    const userCollection = await users();

    let checkUsername = await userCollection.findOne({
      username: creationInfo.username,
    });
    if (checkUsername)
    ...
    if (!createdUser || typeof createdUser === "null")
  ...
  async getUserById(userId) {
    ...
    let userList = await userCollection.findOne(
      { employeeId: userId },
      { firstName: 1, lastName: 1 /*more projections*/ }
    );
  ...
  async updatePut(updationInfo) {
    ...
    if (!updatedInfo || updatedInfo === "null")
      throw new Error(
        `Could not find user with id: ${updationInfo.employeeId}`
      );
    return updatedInfo;
  },
  ...
  async updatePatch(updationInfo) {
    ...
    if (!patchedInfo)
      throw new Error(`Cannot update user with id: ${employeeId}`);
