...
router
  ...
  .post(async (req, res) => {
    ...
    try {
      data = validation.validateBoardingData(
        null,
        data.employeeId,
        data.taskName,
        data.dueDate,
    ...
    try {
      ...
      let existingBoardData = await boardData.getboardingDataByEmpId(
        data.employeeId
      );
      if (!existingBoardData || existingBoardData === null) {
        //create
        let createdBoardUserData = await boardData.createBoardingTask(
          data.employeeId,
          data
        );
        ...
        //update - PUT
        let updatedBoardUserData = await boardData.updatePutBoardingTask(
          existingBoardData,
          data
        );
  ...
  .patch(async (req, res) => {
    ...
    try {
      updateBoardData = validation.validateBoardingDataPatch(
    ...
    try {
      let patchedInfo = await boardData.updatePatchBoardingTask(
        updateBoardData
      );
