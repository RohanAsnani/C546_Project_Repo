...
const checkStr = (str, param, minLen, maxLen, containNum) => {
  ...
  str = str.toLowerCase();
  if (str.length === 0)
    throw new Error(`${param} cannot be empty or be just spaces.`);
  if (containNum === false) {
    if (/\d/.test(str))
...
$("#loginForm").submit(event => {
  ...
  try {
    ...
    if ($("#username").val().includes(" "))
      throw new Error("Username cannot contain spaces in between.");
...
$("#createUser-form").submit(event => {
  ...
  try {
    checkState($("#maritalStatus").val(), "Marital Status", [
      "single",
      "married",
      "seperated",
      "widowed",
  ...
  try {
    checkState($("#department").val(), "Department", [
      "finance",
      "it",
      "human resources",
      "adminstration",
